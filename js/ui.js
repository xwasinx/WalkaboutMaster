...
class UI {
    ...
    renderSession(db) {
        ...
        dueItems.forEach(item => {
            ...
            html += `
                <div class="item-enter bg-white dark:bg-slate-800 p-4 rounded-2xl shadow-sm border border-slate-100 dark:border-white/5 relative overflow-hidden" id="item-${item.id}">
                    <div class="absolute left-0 top-0 bottom-0 w-1 ${colors.bg}"></div>
                    <div class="pl-2 flex flex-col gap-3">
                        <div class="flex items-start justify-between">
                            <div>
                                <div class="text-sm font-bold text-slate-800 dark:text-white">${item.label}</div>
                                <div class="text-[9px] text-slate-400 font-black uppercase mt-1 tracking-widest">Nivel ${item.level}</div>
                            </div>
                            <button data-action="delete" data-id="${item.id}" class="btn-press w-8 h-8 flex items-center justify-center bg-red-100 dark:bg-red-500/10 text-red-500 rounded-xl hover:bg-red-200 dark:hover:bg-red-500/20 transition-colors shrink-0">
